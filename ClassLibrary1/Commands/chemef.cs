...
namespace ClassLibrary1
{
    ...
    public class ChemicalCommandr : ICommand
    {
        public string Command { get; } = "chemef";
        public string[] Aliases { get; } = { "chf" };
        public string Description { get; } = "Manage chemical effects";

        private readonly string chemicalsFolderPathh = Path.Combine(Exiled.API.Features.Paths.Plugins, "Effects");
        ...
        {
            string filePath = Path.Combine(chemicalsFolderPathh, $"{name}.txt");
        ...
        private void ListChemicals(out string response)
        {
            try
            {
                if (!Directory.Exists(chemicalsFolderPathh))
                {
                    Directory.CreateDirectory(chemicalsFolderPathh);
                }

                var files = Directory.GetFiles(chemicalsFolderPathh, "*.txt")
